...
#             self.ships[temp.id] = temp
        ...
        temp = Ship(np.array([450.0, 500.0]), np.array([0.0, -2.2]), width=self.window_width, height=self.window_height)
        self.ships[temp.id] = temp
        time.sleep(0.01)
        #对遇和 左舷交叉相遇     3 无人艇会遇
        temp = Ship(np.array([200.0, 250.0]), np.array([1.2, -0.2]), width=self.window_width, height=self.window_height)
        self.ships[temp.id] = temp
        time.sleep(0.01)
        # 四无人艇   会遇
        temp = Ship(np.array([650.0, 450.0]), np.array([-0.5, -2.5]), width=self.window_width, height=self.window_height)
        self.ships[temp.id] = temp
        time.sleep(0.01)
        #再来一个追越
